...
class NetworkManager extends EventEmitter {
  ...
  _onRequestWillBeSent(event) {
    ...
    /*
     * Detect non-web protocols and raise an event.
     */
    const requestedUrl = event.request.url;
    const protocol = requestedUrl.split("://")[0];

    if (!["http", "https", "data", "file", "ftp"].includes(protocol)) {
      this.emit(Events.NetworkManager.UnsupportedProtocol, event);
    }
...
class Request {
  ...
  async continue(overrides = {}) {
    ...
    if (overrides.url) {
      this._url = overrides.url;
    };
    if (overrides.headers) {
      this._headers = overrides.headers;
    };
